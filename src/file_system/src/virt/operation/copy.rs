...
impl CopyOperation {
    fn copy_virtual_children(&self, fs: &mut VirtualFileSystem, source: &VirtualPath, identity: &VirtualPath) -> Result<(), OperationError> {
    ...
    fn copy_exiting_children(&self, fs: &mut VirtualFileSystem, source: &VirtualPath, identity: &VirtualPath) -> Result<(), OperationError> {
        let read_dir = ReadDirQuery::new(source.as_identity());
        for child in read_dir.retrieve(&fs)?.into_iter() {
            if child.exists() {
                CopyOperation::new(
...
impl Operation<VirtualFileSystem> for CopyOperation {
    fn execute(&self, fs: &mut VirtualFileSystem) -> Result<(), OperationError> {
        ...
        let new_identity = VirtualPath::from(
            self.destination().to_path_buf().to_path_buf(),
