...
use shell::{ cp, rm, touch, mkdir, ls, mv };
        ...
        if let Ok(_) =  stdin().read_line(&mut input) {
            ...
            match App::from_yaml(yaml).get_matches_from_safe(argv) {
                Ok(matches) => {
                    ...
                    if let Some(_matches) = matches.subcommand_matches("debug_virtual_state") {
...
//                        let path = absolute(cwd.as_path(), Path::new(matches.value_of("path").unwrap()));
                        let path = cwd.join(Path::new(matches.value_of("path").unwrap()));
